...
class PathConditionalSigGenerator(ConditionalGenerator):
    def __init__(self, data_size, noise_size, hidden_size, mlp_size, num_layers, activation, order, logsig,
        ...
        if sig_words == "lyndon_words":
            self._sig_words = lyndon_words(conditioning_dim + 1, order)
        else:  # "all_words"
            self._sig_words = all_words(conditioning_dim + 1, order)
        sig_size = len(self._sig_words)

        def signature_fn(x) -> torch.Tensor:
            if sig_attr == "logsignature":
                return logsignature(x, order)
            else:  # "signature"
                return signature(x, order)
